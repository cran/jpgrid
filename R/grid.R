...
code_to_grid <- function(grid_size,
  ...
  if (grid_size == 80000L) {
    ...
    if (grid_size == 10000L) {
      ...
      if (grid_size == 1000L) {
        n_X <- n_X_1km
        n_Y <- n_Y_1km
      } else if (grid_size %in% c(500L, 250L, 125L)) {
        # 500m, 250m, 125m
...
grid_to_code <- function(grid) {
  grid_to_code_impl(grid_size = grid_size(grid),
                    n_X = field(grid, "n_X"),
                    n_Y = field(grid, "n_Y"))
}

grid_to_code_impl <- function(grid_size, n_X, n_Y) {
  if (grid_size == 80000L) {
...
is_grid <- function(x,
  ...
  if (is.null(grid_size)) {
    ...
    grid_size(x) == grid_size_match(grid_size)
  }
}
